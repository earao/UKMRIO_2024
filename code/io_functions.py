...
def make_ixi_siot(S,U,Y,yrs):
    Z = {}
    Y_siot = {}
    for yr in yrs:
        q = np.sum(U[yr],1)+np.sum(Y[yr],1)
        invq = 1/q
        invq[invq == inf]=0
        g = np.sum(S[yr],1)
        invg = 1/g
        invg[invg == inf]=0
        C = np.dot(U[yr],np.diag(invg))
        D = np.dot(S[yr],np.diag(invq))
        Z[yr] = df(np.dot(np.dot(D,C),np.diag(g)),index=S[yr].index,columns=S[yr].index)
        Y_siot[yr] = df(np.dot(D,Y[yr]),index=S[yr].index,columns=Y[yr].columns)
...
def ras2(trs,tcs,to_balance):
    ...
    for _ in range(0, 500):
        ...
        for row in range(0, len(trs)):
            temp.iloc[row,:] = np.multiply(np.divide(temp.iloc[row,:], col_sum_now), np.sum(tcs).values);
                
        row_sum_now = np.sum(temp,1)
        row_sum_now[row_sum_now<=0] = 1
            
        for col in range(0, len(tcs)):
            temp.iloc[:,col] = np.multiply(np.divide(temp.iloc[:,col], row_sum_now), np.sum(trs).values)
