...
coicop_file = data_filepath + 'processed/LCFS/LCF_20231201_retrofit.csv'
coicop_lookup = pd.read_csv(coicop_file, header = 0).fillna(0)

hhspenddata_retrofit = {}
for year in years:
    yr = str(year)
    ...
    # save to dictionary
    hhspenddata_retrofit[year]  = useful_data
    
    
file = os.path.join(results_filepath, 'hhspenddata_retrofit.xlsx')
writer = pd.ExcelWriter(file)

for yr in years:
    hhspenddata_retrofit[yr].to_excel(writer,str(yr))
    print(str(yr) + ' saved to hhspenddata_retrofit2.xlsx')
writer.save()

pickle.dump(hhspenddata_retrofit, open(results_filepath + "hhspenddata_retrofit2.p", "wb" ))
