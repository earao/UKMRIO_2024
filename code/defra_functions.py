...
def makeukresults2023(wd, S,U,Y,newY,coicop_exp_tot2,meta,stressor,direct,indicator,allyears,years):
    defra_foot = {}
    ...
    ghg_props = pd.read_excel(os.path.join(energy_filepath, 'UKenergy2023.xlsx'), sheet_name='ghg_props', header = 0, index_col=0)
...
def makeregionresults2023(S,U,Y,newY,Yregion,meta,stressor,direct,indicator,years,concs_dict,coicop_mult,sic_mult,regpophholdsyr,Region_Name):
    defra_foot = {}
    tempregagg = np.zeros((meta['fd']['len_idx'],meta['reg']['len'])) 
    tempsecagg = np.zeros((meta['fd']['len_idx'],meta['sup_dd']['len_idx']))
    for r in range(0,meta['reg']['len']):
...
def makeregionresults2022(S,U,Y,newY,Yregion,meta,stressor,direct,indicator,years,concs_dict,coicop_mult,sic_mult,regpophholdsyr,Region_Name):
#def makeregionresults2022(S,U,Y,newY,Yregion,meta,stressor,direct,indicator,years,concs_dict,conv):
    defra_foot = {}
    tempregagg = np.zeros((meta['fd']['len_idx'],meta['reg']['len'])) 
    tempsecagg = np.zeros((meta['fd']['len_idx'],meta['sup_dd']['len_idx']))
    for r in range(0,meta['reg']['len']):
    ...
    drct = np.zeros((np.size(direct,0),len(years)))
    pop = np.zeros((2,len(years)))
    nipop = [1686000,1693000,1701000,1709000,1721000,1735000,1752000,1770000,1786000,1799000,1810000,1819000,1827000,1835000,1846000,1857000,1867000,1876000,1885000]
    ...
    for i, yr in enumerate(years):
        ...
        eL_vector = np.dot(e,L)
        eL = np.dot(np.diag(e),L)
        reg = np.zeros((meta['reg']['len'],118))
        ind = np.zeros((meta['sup_dd']['len_idx'],118))
        ...
        adv = np.dot(eL,np.diag(np.sum(bigYregion[:,0:-1],1)))
        adv = adv = np.dot(np.transpose(secagg),(np.dot(adv,prdagg))) 
        
        defra_foot[str(yr)+'_reg'] = df(reg, index = meta['reg']['idx'], columns = Yregion[yr].columns[0:np.size(Yregion[yr],1)-1])
        defra_foot[str(yr)+'_ind'] = df(ind, index = meta['sectors']['ind'], columns = Yregion[yr].columns[0:np.size(Yregion[yr],1)-1])
#        defra_foot[str(yr)+'_adv'] = df(adv, index = meta['sectors']['ind'], columns = meta['sectors']['prd'])
       
        if indicator == 'ghg':
            drct[0,i] = direct.loc['Consumer expenditure - not travel',yr]*np.sum(bigYregion[:,31],0)/np.sum(newY[yr].iloc[:,31],0)
            drct[1,i] = direct.loc['Consumer expenditure - travel',yr]*np.sum(bigYregion[:,59],0)/np.sum(newY[yr].iloc[:,59],0)
        ...
        if indicator == 'nrg':
            drct[0,i] = direct.loc['domestic',yr]*np.sum(np.sum(bigYregion[:,31:35]))/np.sum(np.sum(newY[yr].iloc[:,31:35]))
            drct[1,i] = direct.loc['private transport',yr]*np.sum(bigYregion[:,59],0)/np.sum(newY[yr].iloc[:,59],0)
            
        if indicator == 'blc':
            drct[:,i] = direct.loc[:,yr]*np.sum(bigYregion[:,41],0)/np.sum(newY[yr].iloc[:,41],0)
...
def makeukresults0(S,U,Y,meta,stressor,years):
    defra_foot = {}
    sec = meta['sup_dd']['len_idx']
    for yr in years:
        Z = io.make_Z_from_S_U(S[yr],U[yr])
        bigY = np.zeros(shape = [np.size(Y[yr],0)*2,np.size(Y[yr],1)])
        bigY[np.size(Y[yr],0):np.size(Y[yr],0)*2,0:np.size(Y[yr],1)] = Y[yr]
        ...
        bigstressor[0:np.size(Y[yr],0),:] = stressor[yr]
        e = np.sum(bigstressor,1)/x     
        hhold = np.dot(np.dot(np.diag(e),L),np.diag(np.sum(bigY[:,0:0],1)))
        ...
        temp = np.zeros(shape = [sec,(meta['reg']['len'])*7])
     
        for reg_a in range(0,meta['reg']['len']):
            temp_hhold = np.zeros((106))
            temp_npish = np.zeros((106))
            temp_cgovr = np.zeros((106))
            temp_lgovr = np.zeros((106))
            temp_gfcap = np.zeros((106))
            temp_valua = np.zeros((106))
            temp_chinv = np.zeros((106))
            for reg_b in range(0,meta['reg']['len']):
                temp_hhold = temp_hhold+np.sum(hhold[reg_a*106:(reg_a+1)*106,np.size(Y[yr],0)+reg_b*106:np.size(Y[yr],0)+(reg_b+1)*106],0)
                temp_npish = temp_npish+np.sum(npish[reg_a*106:(reg_a+1)*106,np.size(Y[yr],0)+reg_b*106:np.size(Y[yr],0)+(reg_b+1)*106],0)
                temp_cgovr = temp_cgovr+np.sum(cgovr[reg_a*106:(reg_a+1)*106,np.size(Y[yr],0)+reg_b*106:np.size(Y[yr],0)+(reg_b+1)*106],0)
                temp_lgovr = temp_lgovr+np.sum(lgovr[reg_a*106:(reg_a+1)*106,np.size(Y[yr],0)+reg_b*106:np.size(Y[yr],0)+(reg_b+1)*106],0)
                temp_gfcap = temp_gfcap+np.sum(gfcap[reg_a*106:(reg_a+1)*106,np.size(Y[yr],0)+reg_b*106:np.size(Y[yr],0)+(reg_b+1)*106],0)
                temp_valua = temp_valua+np.sum(valua[reg_a*106:(reg_a+1)*106,np.size(Y[yr],0)+reg_b*106:np.size(Y[yr],0)+(reg_b+1)*106],0)
                temp_chinv = temp_chinv+np.sum(chinv[reg_a*106:(reg_a+1)*106,np.size(Y[yr],0)+reg_b*106:np.size(Y[yr],0)+(reg_b+1)*106],0)
            
            temp[:,reg_a*7+0] = temp_hhold
            temp[:,reg_a*7+1] = temp_npish
            temp[:,reg_a*7+2] = temp_cgovr
            temp[:,reg_a*7+3] = temp_lgovr
            temp[:,reg_a*7+4] = temp_gfcap
            temp[:,reg_a*7+5] = temp_valua
            temp[:,reg_a*7+6] = temp_chinv
            
        defra_foot[yr] = temp  
        
        return defra_foot

def makeukresults(S,U,Y,newY,coicop_exp_tot,meta,stressor,direct,indicator,allyears,years):
    defra_foot = {}
    tempregagg = np.zeros((meta['fd']['len_idx'],meta['reg']['len'])) 
    tempsecagg = np.zeros((meta['fd']['len_idx'],meta['sup_dd']['len_idx']))
    for r in range(0,meta['reg']['len']):
    ...
    for i,yr in enumerate(years): 
        ...
        if indicator == 'co2':
            ...
            coicop[160,i] = coicop[160,i] + direct.loc['Consumer expenditure - travel',yr]
        if indicator == 'nrg':
            coicop[101,i] = coicop[101,i] + direct.loc['domestic',yr]*0.223
            coicop[102,i] = coicop[102,i] + direct.loc['domestic',yr]*0.643
            coicop[103,i] = coicop[103,i] + direct.loc['domestic',yr]*0.010
            coicop[104,i] = coicop[104,i] + direct.loc['domestic',yr]*0.061
            coicop[105,i] = coicop[105,i] + direct.loc['domestic',yr]*0.063
            coicop[160,i] = coicop[160,i] + direct.loc['private transport',yr]
        if indicator == 'wat':
            coicop[97,i] = coicop[97,i] + direct[yr]
...
def makeukresults2021(S,U,Y,newY,coicop_exp_tot,meta,stressor,direct,indicator,allyears,years):
    defra_foot = {}
    tempregagg = np.zeros((meta['fd']['len_idx'],meta['reg']['len'])) 
    tempsecagg = np.zeros((meta['fd']['len_idx'],meta['sup_dd']['len_idx']))
    for r in range(0,meta['reg']['len']):
    ...
    for i,yr in enumerate(allyears):
        ...
        reg = np.zeros((meta['reg']['len'],39))
        ind = np.zeros((meta['sup_dd']['len_idx'],39))
        prd = np.zeros((meta['sup_dd']['len_idx'],39))
        ...
        defra_foot[str(yr)+'_reg'] = df(reg, index = meta['reg']['idx'], columns = Y[yr].columns[0:np.size(Y[yr],1)-1])
        defra_foot[str(yr)+'_ind'] = df(ind, index = meta['sectors']['ind'], columns = Y[yr].columns[0:np.size(Y[yr],1)-1])
        defra_foot[str(yr)+'_prd'] = df(prd, index = meta['sectors']['prd'], columns = Y[yr].columns[0:np.size(Y[yr],1)-1])
...
def makeregionresults(S,U,Y,newY,Yregion,meta,stressor,direct,indicator,years,concs_dict2,conv):
    defra_foot = {}
    tempregagg = np.zeros((meta['fd']['len_idx'],meta['reg']['len'])) 
    tempsecagg = np.zeros((meta['fd']['len_idx'],meta['sup_dd']['len_idx']))
    for r in range(0,meta['reg']['len']):
    ...
    for i, yr in enumerate(years):
        ...
        eL_vector = np.dot(e,L)
        eL = np.dot(np.diag(e),L)
        reg = np.zeros((meta['reg']['len'],313))
        ind = np.zeros((meta['sup_dd']['len_idx'],313))
        ...
        defra_foot[str(yr)+'_reg'] = df(np.dot(reg,concs_dict2['C313_to_C39']), index = meta['reg']['idx'], columns = Y[yr].columns[0:np.size(Y[yr],1)-1])
        defra_foot[str(yr)+'_ind'] = df(np.dot(ind,concs_dict2['C313_to_C39']), index = meta['sectors']['ind'], columns = Y[yr].columns[0:np.size(Y[yr],1)-1])
    
        if indicator == 'ghg':
            drct[0,i] = direct.loc['Consumer expenditure - not travel',yr]*np.sum(bigYregion[:,101],0)/np.sum(newY[yr].iloc[:,101],0)
            drct[1,i] = direct.loc['Consumer expenditure - travel',yr]*np.sum(bigYregion[:,160],0)/np.sum(newY[yr].iloc[:,160],0)
            coicop[101,i] = coicop[101,i] + drct[0,i]
            coicop[160,i] = coicop[160,i] + drct[1,i]
        if indicator == 'co2':
            drct[0,i] = direct.loc['Consumer expenditure - not travel',yr]*np.sum(bigYregion[:,101],0)/np.sum(newY[yr].iloc[:,101],0)
            drct[1,i] = direct.loc['Consumer expenditure - travel',yr]*np.sum(bigYregion[:,160],0)/np.sum(newY[yr].iloc[:,160],0)
            coicop[101,i] = coicop[101,i] + drct[0,i]
            coicop[160,i] = coicop[160,i] + drct[1,i]
        if indicator == 'nrg':
            drct[0,i] = direct.loc['domestic',yr]*np.sum(np.sum(bigYregion[:,101:106]))/np.sum(np.sum(newY[yr].iloc[:,101:106]))
            drct[1,i] = direct.loc['private transport',yr]*np.sum(bigYregion[:,160],0)/np.sum(newY[yr].iloc[:,160],0)
            coicop[101,i] = coicop[101,i] + drct[0,i]*0.223
            coicop[102,i] = coicop[102,i] + drct[0,i]*0.643
            coicop[103,i] = coicop[103,i] + drct[0,i]*0.010
            coicop[104,i] = coicop[104,i] + drct[0,i]*0.061
            coicop[105,i] = coicop[105,i] + drct[0,i]*0.063
            coicop[160,i] = coicop[160,i] + drct[1,i]
        if indicator == 'wat':
            drct[:,i] = direct.loc[:,yr]*np.sum(bigYregion[:,97],0)/np.sum(newY[yr].iloc[:,97],0)
            coicop[97,i] = coicop[97,i] + drct[:,i] 
...
def makeregionresults2021(S,U,Y,newY,Yregion,meta,stressor,direct,indicator,years,concs_dict2,conv,regpophholdsyr,Region_Name):
    defra_foot = {}
    tempregagg = np.zeros((meta['fd']['len_idx'],meta['reg']['len'])) 
    tempsecagg = np.zeros((meta['fd']['len_idx'],meta['sup_dd']['len_idx']))
    for r in range(0,meta['reg']['len']):
    ...
    for i, yr in enumerate(years):
        ...
        reg = np.zeros((meta['reg']['len'],313))
        ind = np.zeros((meta['sup_dd']['len_idx'],313))
        prd = np.zeros((meta['sup_dd']['len_idx'],313))
        
        for ysec in range (0,np.size(Yregion[yr],1)-1):
            reg[:,ysec] = np.dot(np.dot(eL,bigYregion[:,ysec]),regagg)
            ind[:,ysec] = np.dot(np.dot(eL,bigYregion[:,ysec]),secagg)
            prd[:,ysec] = np.dot(np.dot(np.sum(eL,0),np.diag(bigYregion[:,ysec])),prdagg)
            coicop[ysec,i] = np.dot(eL_vector,bigYregion[:,ysec])
        
        defra_foot[str(yr)+'_reg'] = df(np.dot(reg,concs_dict2['C313_to_C39']), index = meta['reg']['idx'], columns = Y[yr].columns[0:np.size(Y[yr],1)-1])
        defra_foot[str(yr)+'_ind'] = df(np.dot(ind,concs_dict2['C313_to_C39']), index = meta['sectors']['ind'], columns = Y[yr].columns[0:np.size(Y[yr],1)-1])
        defra_foot[str(yr)+'_prd'] = df(np.dot(prd,concs_dict2['C313_to_C39']), index = meta['sectors']['prd'], columns = Y[yr].columns[0:np.size(Y[yr],1)-1])
